...
const createNdtValidation = () => [
  ...
  body("MKGD")
    ...
    .isLength({ min: 6 })
    .withMessage("Mật khẩu phải ít nhất 6 ký tự."), // Thêm check độ phức tạp nếu cần
  body("DiaChi")
    .trim()
    .notEmpty()
...
const updateNdtValidation = () => [
  ...maNdtParamValidation(), // Validate MaNDT từ URL
  // Validate các trường trong body tương tự create nhưng là optional
  body("HoTen")
    .optional()
    .trim()
    .notEmpty()
  ...
  body("NgaySinh")
    ...
    .toDate()
    .withMessage("Ngày sinh không hợp lệ (YYYY-MM-DD)."),
  // MKGD không nên cập nhật ở đây, dùng API đổi mật khẩu riêng
  body("DiaChi")
    .optional()
    .trim()
    .notEmpty()
